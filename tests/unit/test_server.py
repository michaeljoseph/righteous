...
class ServerTestCase(ApiTestCase):
    ...
    def test_server_info(self):
        ...
        self.request.assert_called_once_with(
            '/server/ref.js', prepend_api_base=False)
    ...
    def test_start_server(self):
        ...
        self.request.assert_called_once_with(
            '/server/ref/start', method='POST', prepend_api_base=False)

    def test_stop_server(self):
        self.response.status_code = 201
        assert righteous.stop_server('/server/ref')
        self.request.assert_called_once_with(
            '/server/ref/stop', method='POST', prepend_api_base=False)

    def test_delete_server(self):
        self.response.content = '{}'
        assert righteous.delete_server('/server/ref')
        self.request.assert_called_once_with(
            '/server/ref', method='DELETE', prepend_api_base=False)
    ...
    def test_create_server(self):
        ...
        righteous.create_server(
            nickname, 'm1.large',
            create_server_parameters=create_server_parameters)
        self.request.assert_called_once_with(
            '/servers', method='POST', body=expected)
    ...
    def test_set_server_parameters(self):
        ...
        self.request.assert_called_once_with(
            server_href, method='PUT', body=expected,
    ...
    def test_create_and_start_server(self):
        ...
        success, location = righteous.create_and_start_server(
            'arduous', 'm1.small',
            server_template_parameters={'envname': 'arduous'})
        assert success
        ...
        self.request.assert_any_call(
            '/servers', method='POST', body=create_expected)

        # set server parameters
        self.request.assert_any_call(
            new_server_href, method='PUT',
        ...
        # start server
        self.request.assert_any_call(
            new_server_href + '/start', method='POST', prepend_api_base=False)
